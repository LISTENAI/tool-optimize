...
export default async (type: string, configPath: string, onProcess?: (opts: {
    message: string;
    percent: number;
}) => void) => {
    const { cmd } = lisa
    let success = false
    const toolExe = path.join(__dirname, '../../tool/mini_esr_tool.exe')
    const exec = cmd(toolExe, [type, configPath])
